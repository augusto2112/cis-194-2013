  ...
  describe "insert" $ do
    ...
    context "when the log message has a timestamp smaller or equal than root" $
      it "inserts the given LogMessage to the left" $ do
        insert (logMessage 1)  (messageTree 10) `shouldBe`
          Node (messageTree 1)  (logMessage 10) Leaf
        insert (logMessage 10) (messageTree 10) `shouldBe`
          Node (messageTree 10) (logMessage 10) Leaf

    context "when the log message has a timestamp greater than root" $
      it "inserts the given LogMessage to the right" $
        insert (logMessage 10) (messageTree 1) `shouldBe`
          Node Leaf (logMessage 1) (messageTree 10)

  describe "build" $
    it "builds up a MessageTree from a list of log messages" $
      build [ logMessage 1, logMessage 10 ] `shouldBe`
        Node Leaf (LogMessage Info 1 "") (Node Leaf (LogMessage Info 10 "") Leaf)

  describe "inOrder" $
    it "sorts log messages by timestamp" $
      inOrder (Node (messageTree 1) (logMessage 2) (messageTree 3)) `shouldBe`
        [logMessage 1, logMessage 2, logMessage 3]
...
logErrorMessage severity = LogMessage (Error severity)
